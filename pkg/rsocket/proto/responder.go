...
type Responder interface {
	...
	HandleRequestResponse(streamID StreamID, payload Payload) (Payload, error)

	// Called when a new `requestStream` occurs from an Requester.
	HandleRequestStream(streamID StreamID, payload Payload) (<-chan *Result, error)

	// Called when a new `requestChannel` occurs from an RSocketRequester.
	HandleRequestChannel(streamID StreamID, payloads <-chan *Result) (<-chan *Result, error)
